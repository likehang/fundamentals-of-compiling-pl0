...
class Parser(object):
    def parse(self,lex,lev,tx):
        ...
        self.nexttok = None
        self._advance()
        return self.block(lev,tx)# lev = 0  
    ...
    def accept_array(self,lev ,dx): #add
        if self._accept('ID'):
            ...
            if self._accept('NUMBER') or self._accept('ID'):
                ...
                if op == 'NUMBER':
                    num = self.tok.value
                    table.append([name,lev,'ARRAY',[None]*num,[range(dx,dx+num)]])
                    dx+=num
                else:
                    cname = self.tok.value
                    i = self.geti(cname,lev)
                    if i[2] == 'CONST':
                        table.append([name,lev,'ARRAY',[None]*i[3],[range(dx,dx+i[3])]])
                        dx+=i[3]
                    else:
                        self._error()
    ...
    def accept_condition(self,lev):
        if self._accept('ID') or self._accept('NUMBER') or self._accept('ODD'):
            ...
            if op =='ODD':
                ...
                self.expr(lev)
                if self._accept('GE') or self._accept('LE') or self._accept('GT') or self._accept('LT') or self._accpet('NEQ') or self._accept('EQ'):
                    op = self.tok.type
                    self.expr(lev)
                    if op =='EQ':
                        gendo('opr',0,8 )
        ...
        while self._accept('NUMBER') or self._accept('LPAREN') or self._accept('ID') :
            ...
            if op == 'NUMBER':
                ...
                if i[2] == 'VAR':
                    ...
                    if self.nexttok.type == 'DPLUS' :
                        op = self._accept('DPLUS')
                        gendo('lit',0,1)
                        gendo('opr',0,2)
                    elif self.nexttok.type == 'DMINUS':
                        op = self._accept('DMINUS')
                        gendo('lit',0,1)
                        gendo('opr',0,3)
                    else:
                        pass
...
if __name__ == '__main__':
    ...
    e.print_table()
    #'''
    print('start PL0:',file = save)
    print('start PL0:')
    x=0
    for i in gentable :
        x+=1
        print(x, ':',i[0],i[1],i[2] ,file = save )
        print(x, ':',i[0],i[1],i[2] )
    #'''
    print('\nstart PL0 table:' ,file = save)
    print('\nstart PL0 table:' )
    for i in table:
        print(i ,file = save)
        print(i)
